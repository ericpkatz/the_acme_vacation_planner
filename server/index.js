...
const init = async()=> {
  ...
  const [moe, lucy, ethyl, berlin, barcelona, seoul, london] = await Promise.all([
    createUser({ name: 'moe'}),
  ...
  const vacations = await Promise.all([
    ...
    createVacation({ user_id: lucy.id, place_id: barcelona.id, travel_date: '04/01/2024' }),
  ]);
  ...
  app.listen(port, ()=> {
    ...
    console.log(`curl -X POST localhost:${port}/api/users/${ethyl.id}/vacations -d '{"travel_date": "04/01/2025", "place_id": "${seoul.id}"}' -H "Content-Type:application/json"`);
  });
